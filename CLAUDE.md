...
- 🔨 2.3 Database views (~3-4, splitting): ✅ 2.3a data model (migration 0020: collection_properties/note_property_values/collection_views; propertyTypes codec 12 tests; databaseDb + databaseStore) · ✅ 2.3b TABLE view (open a collection as a database via sidebar table icon → editable typed cells for all 8 types, add/rename/retype/delete properties, new-row) · ⬜ 2.3c board/gallery/calendar views · ⬜ 2.3d filters/sorts/grouping/saved-views
...
- ⚠️ Needs a **full `tauri dev` restart** (migration 0020). tsc / **195 tests** (183→195) / cargo / build green. **2.3c (board/gallery/calendar) + 2.3d (filters/sorts/grouping) are the next slices.**
