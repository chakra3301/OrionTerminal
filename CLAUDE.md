...
- 🔨 1.6 Real LSP — foundation shipped 2026-06-13 (1.6a): Rust stdio pipe + JSON-RPC client + manager (probe/launch ts-language-server/pyright/rust-analyzer per root, initialize, auto doc-sync, **semantic diagnostics → Problems**, hover, LSP-first ⌘F12, browser-TS muted when live, project-switch teardown, status-bar indicator, graceful no-server degradation). REMAINING (1.6b, next session): find-all-references, rename, code actions/quick-fixes, organize-imports, completion/signature-help via LSP. Requires user to install the servers (npm i -g typescript-language-server, pyright; rustup component add rust-analyzer).
